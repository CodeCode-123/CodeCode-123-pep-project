...
public class SocialMediaController {
    ...
    public Javalin startAPI() {
        ...
        app.post("/register", this::postAcountHandler);
        app.post("/login", this::postAccountByUsernameAndPasswordHandler);
     ...
     */
    private void postAcountHandler(Context ctx) throws JsonProcessingException {
     ...
     * will contain a JSON representation of an Account, not containing an account_id. In the future, this action
     * may generate a Session token to allow the user to securely use the site. We will not worry about this for now.
     ...
     * account in the response body, including its account_id. The response status should be 200 OK, which is the
     * default.
     * •	If the login is not successful, the response status should be 401. (Unauthorized)
     * @param ctx
     * @throws JsonProcessingException
     */
    private void postAccountByUsernameAndPasswordHandler(Context ctx) throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        Account account = mapper.readValue(ctx.body(), Account.class);
     ...
     */
    private void postMessageHandler(Context ctx) throws JacksonException {
        ObjectMapper mapper = new ObjectMapper();
     ...
     * @param ctx
     */
    private void getMessageByMessageIdHandler(Context ctx) throws SQLException, JsonProcessingException {
        Message message = messageService.retrieveMessageByMessageId(Integer.parseInt(ctx.pathParam("message_id")));
        if (message == null) {
            ctx.json("");
        } else {
            ctx.json(message);
    ...
    private void deleteMessageByMessageIdHandler(Context ctx) {
        Message message = messageService.deleteMessageByMessageId(Integer.parseInt(ctx.pathParam("message_id")));
        if (message == null) {
            ctx.json("");
        } else {
            ctx.json(message);
     ...
     * @throws JacksonException
     */
    private void patchMessageTextHandler(Context ctx) throws JacksonException, SQLException {
        ObjectMapper mapper = new ObjectMapper();
        Message message = mapper.readValue(ctx.body(), Message.class);
